...
struct MarketDataService {
    ...
    init() {
        // Read API key from generated APIKey.swift file (created by build script from Config.xcconfig)
        // Fallback to Info.plist if the generated file isn't available
        let apiKey: String
        
        // First, try to use the generated APIKey.swift file
        // This file is generated by the build script from Config.xcconfig
        apiKey = APIKey.value
        
        self.apiKey = apiKey
        
        // Debug: Output the API key to verify obfuscation is working
        // Note: In production, you may want to remove or conditionally compile this
        print("🔍 DEBUG - API Key loaded from Config.xcconfig via APIKey.swift: \(apiKey)")
        logger.info("API Key successfully loaded from configuration")
    }
    
    private func validateAPIKey() {
        // API key is loaded from Config.xcconfig via Info.plist
        guard !apiKey.isEmpty else {
            logger.error("API_KEY is empty")
            fatalError("API_KEY must be configured in Config.xcconfig.")
        }
    }
